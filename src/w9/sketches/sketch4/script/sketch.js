var Engine = Matter.Engine, //변수로 쓸 코드들
...
function setup() {
  ...
  var ground = Bodies.rectangle(395, 600, 815, 50, {
    ...
    rock = Bodies.polygon(170, 450, 8, 20, rockOptions), //8은 변의 갯수
    anchor = { x: 170, y: 450 },
    elastic = Constraint.create({
      //constrain은 두가지를 고정되게 묶는
      pointA: anchor,
      bodyB: rock,
      length: 0.01,
      damping: 0.01,
...
function draw() {
  background('white');
  beginShape();
  rock.veritices.forEach(each);
}

// create renderer
let elementTarget = document.querySelector('#canvas');
